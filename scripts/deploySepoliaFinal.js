...
async function main() {
  console.log("🚀 Desplegando ActivaChains en Sepolia...");
  
  // IMPORTANTE: Reemplaza YOUR_ALCHEMY_KEY con tu API key real de Alchemy
  ...
  try {
    ...
    const activaToken = await ActivaToken.connect(wallet).deploy("Activa Token", "ACTIVA", ethers.parseEther("1000000"));
    ...
    const activaNFT = await ActivaNFT.connect(wallet).deploy();
    ...
    // Mostrar resumen
    console.log("\n🎉 ¡Despliegue completo exitoso en Sepolia!");
    ...
    console.log("\n📝 Actualiza tu archivo .env.local en el frontend con:");
    console.log(`NEXT_PUBLIC_SEPOLIA_USDC=${usdcAddress}`);
    console.log(`NEXT_PUBLIC_SEPOLIA_ACTIVA_NFT=${activaNFTAddress}`);
    console.log(`NEXT_PUBLIC_SEPOLIA_ACTIVA_TOKEN=${activaTokenAddress}`);
    console.log(`NEXT_PUBLIC_SEPOLIA_MARKETPLACE=${marketplaceAddress}`);
    console.log(`NEXT_PUBLIC_SEPOLIA_REPUTATION=${reputationAddress}`);
